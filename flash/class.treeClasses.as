...
// }}}
// {{{ duplicate()
class_tree.prototype.duplicate = function(node) {
...
class_tree.prototype.idIsInData = function(idsToSearch, actualNode) {
	if (actualNode == undefined) {
		...
		for (i = 0; i < idsToSearch.length; i++) {
			if (actualNode.id == idsToSearch[i]) {
				found = true;
			}	
		}
...
class_tree_pages.prototype.duplicate = function(node) {
	if (this.isValidDuplicate(node)) {
		...
		var tempNode = super.duplicate(node);
		tempNode.attributes.name = newName;
		tempNode.removeIds();
		this.loading = true;
		this.onChange();
		
		_root.phpConnect.send("duplicate_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["new_name", newName], ["type", this.type]]);
	}
};
// }}}
// {{{ renameNode()
class_tree_pages.prototype.renameNode = function(node, newName) {
	this.loading = true;
	_root.phpConnect.send("rename_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["new_name", newName], ["type", this.type]]);
...
class_tree_pages.prototype.move_in = function(node, targetNode) {
	if (this.isValidMove(node, targetNode)) {
		this.loading = true;
		_root.phpConnect.send("move_node_in", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["target_id", targetNode.id], ["type", this.type]]);
...
class_tree_pages.prototype.move_before = function(node, targetNode) {
	if (node != targetNode && this.isValidMove(node, targetNode.parentNode)) {
		this.loading = true;
		_root.phpConnect.send("move_node_before", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["target_id", targetNode.id], ["type", this.type]]);
...
class_tree_pages.prototype.move_after = function(node, targetNode) {
	if (node != targetNode && this.isValidMove(node, targetNode.parentNode)) {
		this.loading = true;
		_root.phpConnect.send("move_node_after", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["target_id", targetNode.id], ["type", this.type]]);
...
class_tree_pages.prototype.copy_in = function(node, targetNode) {
	if (this.isValidCopy(node, targetNode)) {
		var newName =  node.attributes.name + " " + conf.lang.tree_after_copy;
		this.loading = true;
		_root.phpConnect.send("copy_node_in", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["target_id", targetNode.id], ["type", this.type], ["new_name", newName]]);
...
class_tree_pages.prototype.addNode = function(targetNode, type, subType) {
	...
	if (targetNode != null) {
		...
		this.loading = true;
		_root.phpConnect.send("add_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["target_id", targetNode.id], ["type", this.type], ["node_type", type], ["xmldata", add_node_string], ["new_name", new_name]]);
...
class_tree_pages.prototype.getUriById = function(id) {
	...
	while (tempNode != this.data) {
		if (tempNode != this.data.getRootNode()) {
			if (id == tempNode.id && !this.isFolder(tempNode)) {
...
class_tree_page_data.prototype.addNode = function(targetNode, type) {
	...
	if (targetNode != null) {
		...
		this.loading = true;
		this.onChange();
		_root.phpConnect.send("add_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["target_id", targetNode.id], ["type", this.type], ["node_type", add_node_string], ["new_name", new_name]]);
	}	
};
// }}}
// {{{ onDelete()
...
class_tree_files.prototype.renameNode = function(node, newName) {
	_root.phpConnect.send("rename_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["new_name", newName], ["type", this.type]]);
	node.attributes['name'] = newName;
	this.setNodeIds(node, node.parentNode.id);
};
// }}}
// {{{ onDelete()
...
class_tree_files.prototype.copy_in = function(node, targetNode) {
	if (this.isValidCopy(node, targetNode)) {
		_root.phpConnect.send("copy_node_in", [["sid", conf.user.sid], ["wid", conf.user.wid], ["id", node.id], ["target_id", targetNode.id], ["type", this.type], ["new_name", node.attributes.name + " " + conf.lang.tree_after_copy]]);
...
class_tree_files.prototype.addNode = function(targetNode, type) {
	if (targetNode != null) {
		...
		this.loading = true;
		this.onChange();
		_root.phpConnect.send("add_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["target_id", targetNode.id], ["type", this.type], ["node_type", type], ["new_name", newName]]);
	}	
};
// }}}
...
class_tree_colors.prototype.addNode = function(targetNode, type) {
	...
	if (targetNode != null) {
		this.loading = true;
		this.onChange();
		_root.phpConnect.send("add_node", [["sid", conf.user.sid], ["wid", conf.user.wid], ["target_id", targetNode.id], ["type", this.type], ["node_type", type], ["new_name", new_name]]);
	}	
	//this.project.preview();
};
// }}}
