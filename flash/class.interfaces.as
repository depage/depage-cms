...
class_treeDisplayData.prototype.defineButtons = function() {
	...
	//new button
	var button = Object();
	button.symbol = "icon_tree_button_new";
	button.tooltip = conf.lang.buttontip_tree_new;
	button.width = 19;
	button.height = 17;
	// {{{ button.onClick()
	...
	// }}}
	// {{{ button.getEnabledState()
	button.getEnabledState = function() {
		return this._parent.new_nodes.length > 0;
	};
	// }}}
	this.buttons[0].push(button);
	
	//duplicate button
	var button = Object();
	button.symbol = "icon_tree_button_duplicate";
	button.tooltip = conf.lang.buttontip_tree_duplicate;
	button.width = 19;
	button.height = 17;
	// {{{ button.onClick()
	button.onClick = function() {
	...
	//delete button
	var button = Object();
	button.symbol = "icon_tree_button_delete";
	button.tooltip = conf.lang.buttontip_tree_delete;
	button.width = 19;
	button.height = 17;
	// {{{ button.onClick()
	button.onClick = function() {
		...
		if (node != null) {
			...
			while (this.activeNodeIdPrev == null && tempNode != null) {
				if (this.treeObj.isTreeNode(tempNode)) {
					this.activeNodeIdPrev = tempNode.id;
				}
			...
			while (this.activeNodeIdNext == null && tempNode != null) {
				if (this.treeObj.isTreeNode(tempNode)) {
					this.activeNodeIdNext = tempNode.id;
				}
				tempNode = tempNode.nextSibling;
			}
			
			if (node.parentNode.id != undefined) {
				this.activeNodeIdParent = node.parentNode.id;
			}
		...
		if (actualNode.id == id) {
			...
			for (i = 0; i < actualNode.childNodes.length; i++) {
				if (this.treeObj.isTreeNode(actualNode.childNodes[i])) {
					...
					if (found) {
						this.showChildren[actualNode.id] = true;	
						return found;
					}
				}
			}
...
class_treeDisplayData.prototype.onChange = function(node) {
	...
	if (this.treeObj.data != undefined && !this.treeObj.isEmpty()) {
		...
		if (node != undefined) {
			...
			setActiveBy = "node, given directly to onChange function with id '" + node.id + "'";
...
class_treeDisplayData.prototype.getLineNum = function(actualNode) {
	...
	if (!this.isOpen) {
		...
		if (!this.treeObj.isTreeNode(actualNode)) {
			return 0;
		} else if (!this.showChildren[actualNode.id] || !actualNode.hasChildNodes()) {
			return 1;	
		} else {
			for (i = 0; i < actualNode.childNodes.length; i++) {
...
class_treeDisplayData.prototype.getTreeLines = function(treeLinesArray, actualChild, level) {
	if (!this.isOpen) {
		...
		if (this.treeObj.isTreeNode(actualChild)) {
			...
			if (this.showChildren[actualChild.id] || actualChild == this.treeObj.data) {
...
class_treeDisplayData.prototype.setShowChildrenInitLevel = function(actualNode, actualLevel) {
	if (actualNode == undefined) {
		...
		if (this.showChildren[actualNode.id] == undefined) {
			if (actualLevel < this.showChildrenInitLevel) {
				this.showChildren[actualNode.id] = true;
			} else {
				this.showChildren[actualNode.id] = false;
			}
		}
...
class_treeDisplayData.prototype.setShowChildren = function(node, status) {
	this.showChildren[node.id] = status;
	this.redraw();	
};
// }}}
...
class_treeDisplayData_page_data.prototype.setActiveNode = function(node, scrollToActiveNode) {
	...
	this.treeObj.load_prop(this.activeNode.id, this.activeNode);
};
// }}}
// {{{ onChange()
	...
	if (id == null) {
		...
		for (i = 1; i < ids.length - 1; i++) {
			...
			if (tempNode == null) {
				...
				this.showChildren[tempNode.id] = true;
				targetNode = tempNode;	
			}
		}
	}
...
class_treeDisplayData_files.prototype.defineButtons = function() {
	...
	//new folder button
	var button = Object();
	button.symbol = "icon_tree_button_new_folder";
	button.tooltip = conf.lang.buttontip_tree_newfolder;
	button.width = 19;
	button.height = 17;
	// {{{ button.onClick()
	button.onClick = function() {
	...
	var button = Object();
	button.symbol = "icon_tree_button_upload";
	button.tooltip = conf.lang.buttontip_tree_upload;
	button.text = conf.lang.buttontext_tree_upload;
	button.width = 19;
	button.height = 17;
	// {{{ button.onClick()
	button.onClick = function() {
...
class_treeDisplayData_templates.prototype.defineButtons = function() {
	...
	var button = Object();
	button.symbol = "icon_tree_button_release_templates";
	button.tooltip = conf.lang.buttontip_tree_releasetemp;
	button.text = conf.lang.buttontext_tree_releasetemp;
	button.width = 19;
	button.height = 17;
	// {{{ button.onClick()
	button.onClick = function() {
